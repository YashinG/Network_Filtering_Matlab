function [T8, Rpm, Adjv, Dpm, Mv, Z] = DBHTs(D, S, networkFilter)
...
% NB. Updated by YG (31/10/2022) - to allow TMFG or PMFG
%
% Function call: [T8,Rpm,Adjv,Dpm,Mv,Z]=DBHTs(D,S,networkFilter);
%
% Input
...
%
% networkFilter = Filter that was used on the network (== 'PMFG' or 'TMFG')
%
% Output
%
% T8   = (N x 1) cluster membership vector.
% Rpm  = (N x N) adjacency matrix of PMFG.
% Adjv = Bubble cluster membership matrix from BubbleCluster8.
% Dpm  = (N x N) shortest path length matrix of PMFG
% Mv   = (N x Nb) bubble membership matrix. Nv(n,bi)=1 indicates vertex n is a
%           vertex of bubble bi.
% Z    = DBHT hierarchy
%
...
%
%
% Updated by Yashin Gopi (YG) 31/10/2022
% This function was updated by Yashin Gopi to allow the creation of a DBHT
% from a PMFG ot TMFG. The original function only allowed a TMFG.

% --- Addition from YG -----------
if ~exist('networkFilter', 'var')
    networkFilter = 'PMFG';
end

if strcmp(networkFilter, 'PMFG')
    % Use PMFG
    sparse_S = sparse(S);
    PMFG = pmfg(sparse_S);
...
if length(indx) > 1;
    ...
    [v, ci] = find(Mdjv ~= 0);
    Tc(v) = ci;
    clear v ci % Assign discrete cluster memebership of vertices in the converging bubbles.

    Udjv = Dpm * (Mdjv * diag(1./sum(Mdjv ~= 0)));
    Udjv(Adjv == 0) = inf; % Compute the distance between a vertex and the converging bubbles.
...
for n = 1:size(CliqEdge, 1);
    ...
    bleft = CliqEdge(n, 1:2);
    bleft = bleft(d(bleft) ~= -1);
    bright = CliqEdge(n, 1:2);
    bright = bright(d(bright) == -1);
    [vleft, c] = find(Mv(:, (d ~= -1)) ~= 0);
    vleft = setdiff(vleft, vo);
    [vright, c] = find(Mv(:, (d == -1)) ~= 0);
