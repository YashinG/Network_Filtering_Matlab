...
%       inNetworkFilter     = Filter that was used on the network (== 'PMFG' or 'MST')
...
%       stOut.PMFG   = (pAssets x pAssets) PMFG matrix
%       stOut.PMFG_D = (pAssets x pAssets) PMFG filtered distance matrix;
%       stOut.PMFG_S = (pAssets x pAssets) PMFG filtered similarity matrix;
%
%
...
%     CovMatWtd.m           - to use a weighted correlation matrix
%     runMST.m              - to run the MST network filter
%     runPMFG.m             - to run the PMFG network filter
...
% Pre-process returns
processedRets = preProcessRets(inReturns, timeWts, inBlnStdize, inBlnRemMktMode);
...
tempStOut = getClusters(processedRets, timeWts, inNames, inDistMethod, ...
...
for iSample = 1:in_nSim
    ...
    % Pre-process returns
    tempProcessedRets = preProcessRets(tempRets, timeWts, inBlnStdize, inBlnRemMktMode);
    ...
    tempStOut = getClusters(tempProcessedRets, timeWts, inNames, inDistMethod, ...
